...
import {
  ...
  ShoppingCart, Store, Sun, Timer, Users, X, BookOpen, Landmark, Plus,
} from "lucide-react";
...
/** Global quick-create — create the common documents from anywhere. */
function QuickCreate() {
  const router = useRouter();
  const { me } = useV2();
  const [open, setOpen] = useState(false);
  const ref = useDismissable(() => setOpen(false));
  const allItems: { label: string; href: string; section: AppSection }[] = [
    { label: "Invoice", href: "/book/invoices?new=1", section: "invoices" },
    { label: "Customer", href: "/book/customers?new=1", section: "customers" },
    { label: "Bill", href: "/book/purchases?new=1", section: "purchases" },
    { label: "Vendor", href: "/book/vendors?new=1", section: "vendors" },
    { label: "Manual Journal", href: "/book/ledger", section: "ledger" },
  ];
  const items = allItems.filter((i) => canSee(me.role, i.section));
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-primary px-3 py-2 text-sm font-medium text-primary-foreground transition-opacity hover:opacity-90"
      >
        <Plus className="h-4 w-4" /> New
        <ChevronDown className={cn("h-3.5 w-3.5 transition-transform", open && "rotate-180")} />
      </button>
      {open && (
        <div className="absolute left-0 right-0 z-30 mt-1 overflow-hidden rounded-xl border border-border bg-card p-1 shadow-2xl">
          {items.map((i) => (
            <button
              key={i.href}
            ...
            >
              <Plus className="h-3.5 w-3.5 text-primary" /> New {i.label}
            </button>
          ))}
        </div>
      )}
