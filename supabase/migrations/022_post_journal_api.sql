-- Phase 10: allow the public REST API (service role) to post journal entries.
--
-- post_journal_entry guards writes with is_org_member(auth.uid()). The REST API
-- authenticates with an API key (no user JWT), resolves the org from the key
-- hash, and calls this RPC through the service role — so auth.uid() is null.
-- Relax the guard to: enforce membership ONLY when there is a user session.
-- The function is granted to 'authenticated' only, so the sole other caller is
-- the service role, which has already authenticated the key → org.
...
begin
  -- enforce membership only for interactive (JWT) callers; the service-role
  -- API path has already resolved the org from an authenticated API key.
